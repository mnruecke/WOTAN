 ...
 *
 * Copyright (C) 2017-2020 University of Wuerzburg, Experimental Physics 5, Biophysics
...
/* command set */
// 1) select channel
#define  KEY_DAC1a           '1' //Split channel a
#define  KEY_DAC1b           '2' //Split channel b
#define  KEY_DAC2            '3'
#define  KEY_DAC3            '4'
#define  KEY_DAC4            '5'
#define  KEY_SIG_IN          '6'
...
void init_components(void){
    ...
    sample_CH1a_Start(); 
    sample_CH1b_Start();
    ShiftReg_5_WriteRegValue(CLOCK_SHIFT_CH1ab);
    ShiftReg_5_Start();   
    Opamp_1_Start();
    PGA_1_Start();
    PGA_1_SetGain( PGA_1_GAIN_03 );
    PGA_2_Start();
    PGA_2_SetGain( PGA_2_GAIN_03 );
    
    
    // Adjusting effektive output voltage for all 5 channels:
    // (aims at -> 3 V-peak-peak)
    set_dac_range_4V();
    ...
    if (0u != USBUART_GetConfiguration())
    {
        ...
        if (0u != USBUART_DataIsReady())
        {
            ...
            {   
                ...
                // 1) select a channel
                if ( buffer[0] == KEY_DAC1a || buffer[0] == KEY_DAC1b || buffer[0] == KEY_DAC2 || buffer[0] == KEY_DAC3 || buffer[0] == KEY_DAC4 || buffer[0] == KEY_SIG_IN) {
...
void uart_interface(void)
{
    ...
    if((bytenumber > 0 && puttyIn[0] != 'p') || (puttyIn[0] =='p' && bytenumber == size_of_header + size_of_segment)) {
        ...
        /* process firmware commands */
            // 1) select a channel
            if (  puttyIn[0] == KEY_DAC1a || puttyIn[0] == KEY_DAC1b ||  puttyIn[0] == KEY_DAC2 ||  puttyIn[0] == KEY_DAC3 ||  puttyIn[0] == KEY_DAC4 ||  puttyIn[0] == KEY_SIG_IN) {
...
void show_default_message(void)
{
    ...
    sprintf(sms, "1) Press '%c', '%c', '%c', '%c' or '%c' for measuring DAC 1a, 1b, 2..4 or '%c' for measuring GPIO P0.7", KEY_DAC1a, KEY_DAC1b, KEY_DAC2, KEY_DAC3, KEY_DAC4, KEY_SIG_IN );
        UART_1_PutString(sms); UART_1_PutCRLF(1);
...
void run_sequence(void)
{
    ...
    ShiftReg_5_Stop(); ShiftReg_5_WriteRegValue(CLOCK_SHIFT_CH1ab); ShiftReg_5_Start();
...
void set_dac_range_4V(void)
{
    //IDAC8_1_SetRange( IDAC8_1_RANGE_4V ); -> adjusted for 1V only!
