...
uint8 CompTrigger_initVar = 0u;

/* Internal functions definitoin */
static void CompTrigger_trimAdjust(uint8 nibble) ;

/* static CompTrigger_backupStruct  CompTrigger_backup; */
#if (CY_PSOC5A)
    static CompTrigger_LOWPOWER_BACKUP_STRUCT  CompTrigger_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 CompTrigger_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: CompTrigger_Init
********************************************************************************
*
* Summary:
...
void CompTrigger_Init(void) 
{
    ...
    CompTrigger_SetSpeed(CompTrigger_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( CompTrigger_DEFAULT_HYSTERESIS == 0u )
        CompTrigger_CR |= CompTrigger_HYST_OFF;
    #else
        CompTrigger_CR &= (uint8)(~CompTrigger_HYST_OFF);
    #endif /* CompTrigger_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( CompTrigger_DEFAULT_PWRDWN_OVRD == 0u )
            CompTrigger_CR &= (uint8)(~CompTrigger_PWRDWN_OVRD);
        #else 
            CompTrigger_CR |= CompTrigger_PWRDWN_OVRD;
        #endif /* CompTrigger_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    CompTrigger_CR |= CompTrigger_MX_AO;

    /* Set default sync */
    CompTrigger_CLK &= (uint8)(~CompTrigger_SYNCCLK_MASK);
    #if ( CompTrigger_DEFAULT_BYPASS_SYNC == 0u )
        CompTrigger_CLK |= CompTrigger_SYNC_CLK_EN;
    #else
        CompTrigger_CLK |= CompTrigger_BYPASS_SYNC;
    #endif /* CompTrigger_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: CompTrigger_Enable
********************************************************************************
*
* Summary:
...
{
    CompTrigger_PWRMGR |= CompTrigger_ACT_PWR_EN;
    CompTrigger_STBY_PWRMGR |= CompTrigger_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(CompTrigger_restoreReg == 1u)
        {
            CompTrigger_CR = CompTrigger_lowPowerBackup.compCRReg;

            /* Clear the flag */
            CompTrigger_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: CompTrigger_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  CompTrigger_initVar: Is modified when this function is called for the 
...
void CompTrigger_Start(void) 
{
    ...
    if ( CompTrigger_initVar == 0u )
    {
        CompTrigger_Init();
        
        CompTrigger_initVar = 1u;
    }   
...
/*******************************************************************************
* Function Name: CompTrigger_Stop
********************************************************************************
*
* Summary:
...
void CompTrigger_Stop(void) 
{
    ...
    CompTrigger_PWRMGR &= (uint8)(~CompTrigger_ACT_PWR_EN);
    CompTrigger_STBY_PWRMGR &= (uint8)(~CompTrigger_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        CompTrigger_restoreReg = 1u;

        /* Save the control register before clearing it */
        CompTrigger_lowPowerBackup.compCRReg = CompTrigger_CR;
        CompTrigger_CR = CompTrigger_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: CompTrigger_SetSpeed
********************************************************************************
*
* Summary:
...
void CompTrigger_SetSpeed(uint8 speed) 
{
    ...
    CompTrigger_CR = (CompTrigger_CR & (uint8)(~CompTrigger_PWR_MODE_MASK)) |
                           (speed & CompTrigger_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == CompTrigger_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            CompTrigger_TR = CompTrigger_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            CompTrigger_TR0 = CompTrigger_HS_TRIM_TR0;
            CompTrigger_TR1 = CompTrigger_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            CompTrigger_TR = CompTrigger_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            CompTrigger_TR0 = CompTrigger_LS_TRIM_TR0;
            CompTrigger_TR1 = CompTrigger_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: CompTrigger_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( CompTrigger_WRK & CompTrigger_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: CompTrigger_trimAdjust
********************************************************************************
*
* Summary:
...
static void CompTrigger_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = CompTrigger_WRK & CompTrigger_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                CompTrigger_TR |= CompTrigger_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                CompTrigger_TR0 |= CompTrigger_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                CompTrigger_TR |= CompTrigger_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                CompTrigger_TR1 |= CompTrigger_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((CompTrigger_WRK & CompTrigger_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: CompTrigger_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 CompTrigger_ZeroCal(void) 
{
    ...
    tmpSW0 = CompTrigger_SW0;
    tmpSW2 = CompTrigger_SW2;
    tmpSW3 = CompTrigger_SW3;

     /* Clear routing for inP, retain routing for inN */
    CompTrigger_SW0 = 0x00u;
    CompTrigger_SW2 = 0x00u;
    CompTrigger_SW3 = tmpSW3 & (uint8)(~CompTrigger_CMP_SW3_INPCTL_MASK);

    /* Preserve original configuration
     * - turn off Hysteresis
     * - set calibration bit - shorts inN to inP
    */
    tmpCR = CompTrigger_CR;
    CompTrigger_CR |= (CompTrigger_CAL_ON | CompTrigger_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        CompTrigger_TR = CompTrigger_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        CompTrigger_TR0 = CompTrigger_DEFAULT_CMP_TRIM;
        CompTrigger_TR1 = CompTrigger_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (CompTrigger_CR & CompTrigger_PWR_MODE_MASK) == CompTrigger_PWR_MODE_FAST)
    {
        CompTrigger_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        CompTrigger_trimAdjust(1u);
		CompTrigger_trimAdjust(0u);
    }
   
    /* Restore Config Register */
    CompTrigger_CR = tmpCR;
    
    /* Restore routing registers for inP */
    CompTrigger_SW0 = tmpSW0;
    CompTrigger_SW2 = tmpSW2;
    CompTrigger_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) CompTrigger_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)CompTrigger_TR1 << 8u) | (CompTrigger_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: CompTrigger_LoadTrim
********************************************************************************
*
* Summary:
...
void CompTrigger_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: CompTrigger_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: CompTrigger_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
