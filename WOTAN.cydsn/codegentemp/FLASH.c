...
*******************************************************************************/
#if (CY_PSOC3)
    cystatus FLASH_Write(const uint8 srcBuf[], const uint8 eepromPtr[], uint16 byteCount)
...
#endif /* (!CY_PSOC3) */
{
    ...
    #if (CY_PSOC3)
        eeOffset &= FLASH_CODE_ADDR_MASK;
    #endif /* (CY_PSOC3) */

    /* Make sure, that eepromPtr[] points to ROM */
    #if (CY_PSOC3)
        if (((uint32)eepromPtr >= FLASH_CODE_ADDR_OFFSET) &&
            (((uint32)eepromPtr + byteCount) <= FLASH_CODE_ADDR_END))
    #else
        if (((uint32)eepromPtr + byteCount) <= FLASH_FLASH_END_ADDR)
    #endif /* (CY_PSOC3) */
    {
        #if (!CY_PSOC4)
            (void)CySetTemp();
        ...
        rowId = (eeOffset / CY_FLASH_SIZEOF_ROW) % FLASH_ROWS_IN_ARRAY;
